...
export class Network {
 socket:Socket|null=null;side=0;code='';latest:Snapshot|null=null;connected=false;recoveryFailed=false;error='';lastReceived=0;rtt=0;token='';mode:Snapshot['mode']='multiplayer';
 ...
 async connect(_url:string,kind:'create'|'join',code='',mode:Snapshot['mode']='multiplayer'){
  this.close();this.error='';this.recoveryFailed=false;this.latest=null;
  ...
  socket.on('connect',()=>{if(joined)socket.timeout(7000).emit('join',{code:this.code},(err:Error|null,r:{error?:string;side:number;mode:Snapshot['mode']})=>{if(err||r?.error){this.error=r?.error||'Your room could not reconnect. Return to the menu.';this.recoveryFailed=true;return;}this.side=r.side;this.mode=r.mode;this.connected=true;this.recoveryFailed=false;this.error='';this.buffer=[];this.onReconnect();});});
  return new Promise<void>((resolve,reject)=>{
   ...
   socket.on('connect_error',(error:Error)=>{if(joined&&error.message.includes('already connected')){if(this.retry)clearTimeout(this.retry);this.retry=setTimeout(()=>{if(this.socket===socket)socket.connect();},2000);}this.error=error.message.includes('Session')?error.message:'The court is unavailable. Please retry shortly.';if(joined&&error.message.includes('Session')&&!error.message.includes('already connected'))this.recoveryFailed=true;if(error.message.includes('expired'))try{sessionStorage.removeItem('rally-session');}catch{}if(!joined){clearTimeout(timeout);socket.disconnect();reject(new Error(this.error));}});
 ...
 async browserCamera(preserve=false){if(!this.connected||!this.socket)throw new Error('Reconnect to the room before starting your camera.');const result=await this.socket.timeout(5000).emitWithAck('browser-camera',{preserve});if(!result?.ok)throw new Error('Browser tracking is unavailable on this server.');}
