...
export interface Room {code:string;mode:Mode;players:(Player|null)[];game:Game;phase:Phase;countdown:number;reason:string;updated:number;replays:Set<number>;grades:GradeJob[]}
...
export class Rooms {
 ...
 create(token:string,socketId:string,mode:Mode='multiplayer'){
  ...
  const room:Room={code,mode,players:[player(token,socketId),mode==='multiplayer'?null:player(`${token}:partner`,socketId,1)],game:new Game(Math.random,true),phase:'lobby',countdown:3,reason:'',updated:Date.now(),replays:new Set(),grades:[]};
 ...
 join(code:string,token:string,socketId:string){
  ...
  room.players[side]=player(token,socketId,side);if(room.mode!=='multiplayer')room.players[1]=player(`${token}:partner`,socketId,1);
 ...
 disconnect(room:Room,side:number){
  ...
  if(room.phase==='playing'||room.phase==='countdown'){room.phase='paused';room.reason='A player disconnected. Waiting for them to return.';}
 }
